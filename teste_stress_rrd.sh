...
if [ ! -e "$RRD_DB" ]; then
    ...
    rrdtool create $RRD_DB \
        --start N --step 10 \
        DS:cpu_load_1min:GAUGE:20:0:100 \
        DS:mem_usage:GAUGE:20:0:100 \
        DS:disk_io:GAUGE:20:0:U \
...
# Executa 64 estressores de CPU por 10 minutos e atualiza o RRD
echo "Executando 64 estressores de CPU por 10 minutos:"
sudo stress-ng --cpu 64 --timeout 10m --metrics-brief
...
# Executa 32 estressores de CPU com método FFT por 20 minutos e atualiza o RRD
echo "Executando 32 estressores de CPU com método FFT por 20 minutos:"
sudo stress-ng --cpu 32 --cpu-method fft --timeout 20m --metrics-brief
update_rrd  # Atualiza o RRD após o teste de estresse
...
# Executa 40 estressores de disco até atingir 800000 operações bogo e atualiza o RRD
echo "Executando 40 estressores de disco até 800000 operações bogo:"
sudo stress-ng --hdd 40 --hdd-ops 800000 --metrics-brief
update_rrd  # Atualiza o RRD após o teste de estresse de disco

# Exibe o tempo de atividade novamente após o terceiro teste
echo "Tempo de atividade após o terceiro teste:"
uptime

# Executa 32 estressores de CPU, 16 estressores de I/O e 8 estressores de memória virtual com 8GB por 10 minutos e atualiza o RRD
echo "Executando 32 estressores de CPU, 16 estressores de I/O e 8 estressores de memória virtual (8GB) por 10 minutos:"
sudo stress-ng --cpu 32 --io 16 --vm 8 --vm-bytes 8G --timeout 10m --metrics-brief
update_rrd  # Atualiza o RRD após o teste de estresse
